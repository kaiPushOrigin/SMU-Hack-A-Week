...
var err_msgs = "";
var key_array =[];
var second_key_array=[];
	
	
		key_array = js_var.split(",");
	
		second_key_array = second_js_var.split(",");

	if(second_key_array.length>0)
	{

		for(var i = 0; i<second_key_array.length ; i++)
		{
			if (second_key_array[i] === ",") second_key_array.splice(i, 1);
			if (second_key_array[i] === "") second_key_array.splice(i, 1);
		}	
	}
	else{
		throw "Please add an interest";
	}
	...
	if(key_array.length>0)
	{
		...
		{
			if (key_array[i] === ",") key_array.splice(i, 1);
			if (key_array[i] === "") key_array.splice(i, 1);
		}
	}
	else{
	...
	function showMap(coords) {
		...
		//if(second_key_array.length >1)
		//{
			addNearbyPlacesAgain(map, googleLatAndLong);
	...
	function addNearbyPlacesAgain(map, googleLatAndLong){
